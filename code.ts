  ...
  if (msg.type === 'ping') {
    ...
    const nodes = (msg.nodeIds as string[])
      ...
      .filter((n): n is SceneNode =>
        ...
        n.type !== 'PAGE' &&
        'parent' in n &&
        isOnCurrentPage(n as SceneNode),
      );
    console.log(`[audit] navigate-to: requested=${(msg.nodeIds as string[]).length} resolved=${nodes.length} currentPage="${figma.currentPage.name}"`);
    if (nodes.length === 0) {
      figma.ui.postMessage({
